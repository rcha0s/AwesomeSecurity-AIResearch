...
> Ranked, source-cited takeaways across Security and AI. Updated 2026-07-27.

## 📈 Ranked findings

### AsyncAPI npm compromise: import-time payload defeats --ignore-scripts · `70.12`
...
### Phantom Squatting: attackers register the domains LLMs hallucinate · `66.3`
...
### ToolHive MCP SSRF: host-side discovery runs outside the sandbox it enforces · `65.4`
...
### Better Models, Worse Tools: SOTA models regress on non-native tool schemas · `65.25`
...
### How the Claude Code team designs its harness: tool minimalism, incident-driven evals, system-prompt compaction, and an auto-mode permission classifier · `62.25`
...
### Server-side encrypted compaction: porting Codex's Responses-API compaction protocol into other harnesses (Pi) · `62.25`
...
### Goal-persistent agents: a frontier model built a bespoke zlib fuzzing lab in a day · `61.65`
...
### ShadowPickle: pickle-VM import tricks evade ten model scanners and four model hubs · `60.22`
...
### Agent skill security is a lifecycle problem, not just a runtime one (SkillSec-Eval) · `59.85`
...
### Committed git symlinks + misleading approval dialogs let AI coding assistants read/write files outside the workspace (Wiz 'GhostApproval') · `59.85`
...
### GigaWiper: modular destructive malware that fakes ransomware · `59.25`
...
### TuxBot v3: an LLM-assisted IoT botnet shipped with the model's safety disclaimer and raw chain-of-thought still in the source · `58.65`
...
### Oh My Posh: a directory name runs commands, because the prompt re-renders the resolved path through a template engine whose funcmap has cmd · `58.5`
...
### Kemp LoadMaster pre-auth RCE: uninitialized heap + missing null byte (CVE-2026-8037) · `57.85`
...
### Omnigent: an open-source meta-harness over Claude Code, Codex, Cursor · `57.1`
...
### SourTrade: browser reassembles a Bun-based executable from split parts, defeating hash-based detection with per-session builds · `56.7`
...
### A malicious federated-learning aggregator can backdoor a QA model without ever seeing client data · `53.8`
- **Topic:** ai-security / Model Supply Chain
...
### QuantGuard: a pre-quantization defense against backdoors that only wake up after you quantize · `53.2`
- **Topic:** ai-security / Model Supply Chain
...
<sub>Generated by scripts/generate_skills.py on 2026-07-27.</sub>
